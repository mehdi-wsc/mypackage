...
def test_reponse():
    ...
    infra_builder_terraform_response = subprocess.Popen(['python', 'mypackage_wsc/infra_builder_terraform.py'], stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    test_2 = str(infra_builder_terraform_response.communicate()[0])
    assert test_2 == "b'Missing account argument,Wedeployer Can not run without Account\\n'"
